...
public class TBoxServer extends IModuleCallback.Stub implements IConnStateListener {
    public static int[] DATA = new int[30];
    private static TBoxServer INSTANCE = new TBoxServer();
    ...
    private class Runnable_Update implements Runnable {
        ...
        public void run() {
            if (this.updateCode >= 0 && this.updateCode < 30) {
                TBoxServer.updateNotify(this.updateCode, this.ints, this.flts, this.strs);
            }
        }
    }
    ...
    public static TBoxServer getInstance() {
        return INSTANCE;
    }
    ...
    public void onConnected(IRemoteToolkit iRemoteToolkit) {
        try {
            ...
            TBoxServer instance = getInstance();
            for (int i = 1; i < 30; i++) {
